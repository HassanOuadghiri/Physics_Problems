...
Alice is moving along a path described by

$$
A(t) = (2+t,\ 8-3t)
$$

and Bob is moving along a path

$$
B(t) = (2t-1,\ 2t+2)
$$

Determine if their paths intersect. If yes, determine when and where they will collide. If not, determine the minimum distance between them and when it occurs.
...
A river flows east at $2\ \text{m/s}$. A boat that can travel at $5\ \text{m/s}$ in still water wants to go directly north across the river. In what direction (angle) should it head? How long will it take to cross the river if it is 200 meters wide?
